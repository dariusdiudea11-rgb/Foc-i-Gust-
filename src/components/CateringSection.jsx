      ...
      <div ref={ref} className="relative max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-16 items-start">
        ...
        <motion.div className="md:col-span-7"
          ...
          <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-8 backdrop-blur-sm">
            <h3 className="text-2xl text-[#faf3e8] mb-8"
              style={{ fontFamily: '"DM Serif Display", serif' }}>Trimite o cerere</h3>
            <form onSubmit={submit} className="flex flex-col gap-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <input name="nume" value={form.nume} onChange={handle} placeholder="Nume" required className={fieldClass}/>
                <input name="telefon" type="tel" value={form.telefon} onChange={handle} placeholder="Telefon" required className={fieldClass}/>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <input name="email" type="email" value={form.email} onChange={handle} placeholder="Email" required className={fieldClass}/>
                <input name="data" type="date" value={form.data} onChange={handle} required className={fieldClass}/>
              </div>
              <div className="relative">
                <select name="persoane" value={form.persoane} onChange={handle} required
