...
export default function TarguriList({ targuri, onAdd, onDelete }) {
  ...
  return (
    <div style={{ maxWidth: '900px' }}>
          ...
          {STATUS_FILTERS.map(f => (
            <button key={f.value} onClick={() => setFilter(f.value)}
              style={{
                ...
                background:   filter === f.value ? 'rgba(142,31,27,0.15)' : 'rgba(255,255,255,0.04)',
                border:       `1px solid ${filter === f.value ? 'rgba(142,31,27,0.3)' : 'rgba(255,255,255,0.06)'}`,
                color:        filter === f.value ? '#8E1F1B' : 'rgba(255,255,255,0.5)',
                cursor:       'pointer',
      ...
      {/* Add form inline */}
      {showForm && (
        <div style={{ background: '#2E2A24', border: '1px solid rgba(142,31,27,0.2)', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#F5EFE3', margin: '0 0 20px' }}>Târg nou</h3>
